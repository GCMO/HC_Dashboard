...
const patientData = {
  patients: [
    ...
    },
    {
      ...
      contact: '(955) 676-1076',
      name: 'Tony Herrera',
      title: 'Had a big caugh, prescribed RenYe Concoxion',
      total: 85,
      avatar: '/images/avatars/2.png',
      patientStatus: 'Paid',
      balance: 0,
    ...
    },
    {
      ...
      contact: '(832) 323-6914',
      name: 'Kevin Patton',
      title: 'Reason For Visit: Constipation',
      total: 85,
      avatar: '/images/avatars/3.png',
      patientStatus: 'Sent',
      balance: 0,
    ...
    },
    {
      ...
      contact: '(970) 982-3353',
      name: 'Mrs. Julie Donovan MD',
      title: 'pulse slow, treated stomach & lung meridian',
      total: 85,
      avatar: '/images/avatars/4.png',
      patientStatus: 'Draft',
      balance: '$815',
    ...
    },
    {
      ...
      contact: '(511) 938-9617',
      name: 'Amanda Phillips',
      title: 'moximabation and cupping',
      total: 85,
      avatar: '',
      avatarColor: 'secondary',
      patientStatus: 'Paid',
    ...
    },
    {
      ...
      contact: '(504) 859-2893',
      name: 'Christina Collier',
      title: 'Reduce inflamation with Lungs and Heart M.',
      total: 85,
      avatar: '',
      avatarColor: 'success',
      patientStatus: 'Draft',
...
mock.onGet('/apps/patient/patients').reply(config => {
  const { q = '', status = '', dates = [] } = config.params ?? ''
  const queryLowered = q.toLowerCase()

  const filteredData = patientData.patients.filter(patient => {
    if (dates.length) {
      const [start, end] = dates
      const filtered = []
      const range = getDateRange(start, end)
      const patientDate = new Date(patient.issuedDate)
      range.filter(date => {
        const rangeDate = new Date(date)
        if (
          patientDate.getFullYear() === rangeDate.getFullYear() &&
          patientDate.getDate() === rangeDate.getDate() &&
          patientDate.getMonth() === rangeDate.getMonth()
        ) {
          filtered.push(patient.id)
        }
      })
      if (filtered.length && filtered.includes(patient.id)) {
        return (
          (patient.companyEmail.toLowerCase().includes(queryLowered) ||
            patient.name.toLowerCase().includes(queryLowered) ||
            String(patient.id).toLowerCase().includes(queryLowered) ||
            String(patient.total).toLowerCase().includes(queryLowered) ||
            String(patient.balance).toLowerCase().includes(queryLowered) ||
            patient.dueDate.toLowerCase().includes(queryLowered)) &&
            patient.patientStatus.toLowerCase() === (status.toLowerCase() || patient.patientStatus.toLowerCase())
        )
      }
    } else {
      return (
  ...
  return [
    ...
    {
      params: config.params,
      patientAllData: patientData.patients,
...
mock.onGet('apps/patient/single-patient').reply(config => {
  const { id } = config.params
  const patientData = patientData.patients.filter(patient => patient.id === parseInt(id, 10))
...
mock.onGet('/apps/patient/allpatients').reply(() => {
  const allPatients = patientData.patients.map(patient => {
    const { address, email, country, contact, name, title } = patient
  ...
  return [200, allPatients.slice(0, 5)]
})
...
mock.onDelete('/apps/patient/delete').reply(config => {
  // Get invoice id from URL
  const patientId = Number(config.data)
  const patientIndex = data.patients.findIndex(t => t.id === patientId)
